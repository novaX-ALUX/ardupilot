...
BIN_DIR="${AP_BUILD_DIR}/${BOARD_NAME}/bin"
...
if [[ "${VEHICLE}" == "AP_Periph" ]]; then
    NOVAX_VERSION="${NOVAX_VERSION:-$(cat "${BOARD_DIR}/VERSION" 2>/dev/null \
...
MANIFEST_PATH="${RELEASE_DIR}/manifest_${OUT_BASE}.txt"   # release.sh looks for manifest_<apj basename>.txt
{
    ...
    echo "source_tree=${AP_ROOT}"
    echo "board_config=${BOARD_DIR}/ardupilot"
